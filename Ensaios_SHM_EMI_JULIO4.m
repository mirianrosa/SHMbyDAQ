...
num_ensaios = 1; %parametros{2};         % Número de ensaios (proposta: 10)
%num_ensaios = str2num(num_ensaios);
medidas = 1;%parametros{3};             % Número medidas por ensaio (proposta: 3)
%medidas = str2num(medidas);
num_pontos_sinal = 250000;%parametros{4};    % Número de pontos sinal aleatório (proposta: 250000)
%num_pontos_sinal = str2num(num_pontos_sinal);
f_chirp = 30000;%parametros{5};             % Frequência máxima sinal chirp (proposta: 30000)
...
for i=1:num_ensaios
    ...
    if (mean(abs(data_aleat))>8e-005)    % Checar se houve erro da medida
        ...
        out_aleat(:,(j+(i-1)*medidas))= data_aleat;
        in_aleat(:,(j+(i-1)*medidas))= sinal_excit_aleat;
    ...
    out_chirp(:,(j+(i-1)*medidas))= data_chirp;
    in_chirp(:,(j+(i-1)*medidas))= sinal_excit_chirp;
    ...
    out_idinput(:,(j+(i-1)*medidas))= data_idinput;
    in_idinput(:,(j+(i-1)*medidas))= sinal_excit_idinput_band125;
